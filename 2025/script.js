...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var StName = ['石閔瀚' ,'殷培軒' ,'許力仁', '陳宇睿', '陳柏亘', '黃立寰', '楊尹盷', '趙宥翔', '蔡承浚', '戴誌翔', '方心妤', '方采晴', '王昱晰', '李品靜', '林子媃', '林恩郁', '許渝繐', '黃云柔', '楊喻茜', '劉昕妮', '蔡予瑄', '賴彥汝', '江長澤', '吳泓均', '林祈安', '林鉑承', '黃昭宸','N','N','N'];
  ...
  for (let i = 0; i < 70; i++) {
	...
	if ( i<30){
		...
		button.innerHTML = `<img src="images/0.png" alt=""><span>${i - 39} (${0})</span>`;
	}
      ...
      .then(data => {
        data[0].forEach((value, index) => {
          if (index < 30) { // 確保只處理前30個數據
			...
			button.innerHTML = `<img src="images/${value}.png" alt=""><span>${index + 1}  ${StName[i]} (${value})</span>`;
          }
        });
      })
      .catch(error => console.error('Error:', error));
